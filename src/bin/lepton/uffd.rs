...
use lepton::uffd::{UffdCore, UffdOptions, UffdService};
...
pub fn run_uffd_service(args: UffdArgs) -> Result<()> {
    ...
    let signal_thread = std::thread::Builder::new()
        .name("lepton_uffd_signal".to_string())
        .spawn(move || {
            if let Some(signal) = signals.forever().next() {
                tracing::info!("received signal {signal}, stopping lepton uffd service");
                signal_service.stop();
            }
        })
    ...
    runtime.enable_all().thread_name("lepton_uffd");
