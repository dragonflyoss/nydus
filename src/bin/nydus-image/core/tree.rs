...
impl Tree {
    ...
    /// Apply new nodes (upper layer, array generated by BFS) to node tree (lower layer) with DFS.
    ///
    ...
    pub fn apply(
        ...
        // The only one success situation of depth == target_paths_len
        ...
        // The fail exit of DFS
        if depth >= target_paths_len {
            return Ok(false);
        }

        // Don't search if path recursive depth out of target path
        // The success exit of DFS
        if depth == target_paths_len - 1 {
            let mut node = target.clone();
            match self
                .children
                .binary_search_by_key(&target_paths[depth], |child| {
                    child.node.name().to_os_string()
                }) {
                Ok(idx) => {
                    node.overlay = Overlay::UpperModification;
                    self.children[idx] = Tree {
                        node,
                        children: self.children[idx].children.clone(),
                    };
                }
                Err(_) => {
        ...
        {
            return self.children[idx].apply(target, handle_whiteout, whiteout_spec);
        }
