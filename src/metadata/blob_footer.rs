...
pub const LEPTON_BLOB_FOOTER_MAGIC: u32 = 0x4c46_5452;
pub const LEPTON_BLOB_FOOTER_SIZE: usize = 4096;
pub const LEPTON_BLOB_FOOTER_ALIGNMENT: u64 = EROFS_BLOCK_SIZE as u64;

const LEPTON_BLOB_FOOTER_SUPPORTED_FEATURES: u32 = 0;
...
impl BlobFooter {
    pub fn new(
        ...
        let mut footer = Self {
            magic: LEPTON_BLOB_FOOTER_MAGIC,
            features: 0,
            crc32: 0,
            reserved0: 0,
    ...
    pub fn parse_from_tail(data: &[u8]) -> Result<Self> {
        if data.len() < LEPTON_BLOB_FOOTER_SIZE {
            bail!("blob too small for lepton footer");
        }
        let footer_offset = data.len() - LEPTON_BLOB_FOOTER_SIZE;
    ...
    pub fn read_from_path(path: &Path) -> Result<Self> {
        ...
        let file_size = file
            ...
            .len();
        if file_size < LEPTON_BLOB_FOOTER_SIZE as u64 {
            bail!("blob too small for lepton footer: {}", path.display());
        }
        file.seek(SeekFrom::Start(file_size - LEPTON_BLOB_FOOTER_SIZE as u64))
            .with_context(|| format!("failed to seek blob footer: {}", path.display()))?;
        let mut data = [0u8; LEPTON_BLOB_FOOTER_SIZE];
    ...
    pub fn footer_offset(file_size: u64) -> Result<u64> {
        file_size
            .checked_sub(LEPTON_BLOB_FOOTER_SIZE as u64)
            .context("blob too small for lepton footer")
    }

    fn from_bytes(data: &[u8]) -> Result<Self> {
        if data.len() != LEPTON_BLOB_FOOTER_SIZE {
            bail!("invalid lepton footer size: {}", data.len());
        }
        if data[64..].iter().any(|byte| *byte != 0) {
            bail!("lepton footer reserved extension area must be zero");
        }
        let footer = Self {
        ...
        if footer.crc32 != actual {
            bail!("lepton footer crc32 mismatch");
        }
        Ok(footer)
    }

    fn to_bytes(self) -> [u8; LEPTON_BLOB_FOOTER_SIZE] {
        let mut data = [0u8; LEPTON_BLOB_FOOTER_SIZE];
    ...
    fn validate_common(&self) -> Result<()> {
        if self.magic != LEPTON_BLOB_FOOTER_MAGIC {
            bail!("invalid lepton footer magic");
        }
        if self.features & !LEPTON_BLOB_FOOTER_SUPPORTED_FEATURES != 0 {
            bail!("unsupported lepton footer features: {:#x}", self.features);
        }
    ...
    fn validate_layout(&self, footer_offset: u64) -> Result<()> {
        ...
        for (name, value) in [
            ...
            if value % LEPTON_BLOB_FOOTER_ALIGNMENT != 0 {
                bail!("lepton footer {name} is not 4KiB aligned");
            }
        }
...
mod tests {
    ...
    fn footer_round_trips_and_validates_crc32() {
        ...
        assert_eq!(LEPTON_BLOB_FOOTER_SIZE, 4096);
    }
