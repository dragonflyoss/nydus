...
impl UffdService {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        info!(
            "nydus uffd service stopping, waiting for {} connection(s)",
            connections.len()
        );
        ...
        let _ = fs::remove_file(&self.socket_path);
        info!("nydus uffd service stopped");
        Ok(())
    }
}
...
impl UffdConn {
    ...
    async fn handle_handshake(
        ...
        for (idx, region) in regions.iter().enumerate() {
            debug!(
                "nydus uffd region[{idx}]: virt_addr={:#x} size={:#x} offset={:#x} fault_size={:#x} prot={} flags={}",
                region.virt_addr,
                region.size,
                region.offset,
                region.fault_size,
