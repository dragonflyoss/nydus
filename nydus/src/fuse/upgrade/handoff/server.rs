...
fn accept_loop(
    ...
    // Connections are handled serially through close or deadline; an INFO
    // timeout behind another connection is not evidence of daemon exit.
    loop {
        let mut poll = [
            libc::pollfd {
        ...
        let stream = match listener.accept() {
            ...
            Err(err) => {
                warn!(
                    "fuse control endpoint lost; file service may remain active, \
                     but INFO and further upgrades are unavailable: {err}"
                );
                break;
            }
        };
...
mod tests {
    ...
    use crate::fuse::upgrade::transfer::{
        receive_opaque_transfer, OpaqueSessionTransfer, SessionTransferMetadata,
    };
    ...
    fn begin_handoff(
        ...
        let transfer = receive_opaque_transfer(
            &mut successor,
        ...
        .unwrap();
        assert_eq!(
            SessionTransferMetadata::decode(&transfer.metadata)
                .unwrap()
                .session_id,
            session_id
        );
    ...
    #[test]
    fn info_reports_standalone_and_protected_sessions() {
        for session_id in [None, Some(uuid::Uuid::from_u128(7))] {
            let (_dir, path, server, background, _kernel) = started_server(session_id);
    ...
    fn complete_ready_reports_commit_and_retires_predecessor() {
        for session_id in [None, Some(uuid::Uuid::from_u128(9))] {
            let (_dir, _kernel, mut successor, transfer, handoff, background) =
                begin_handoff(session_id);
    ...
    fn explicit_abort_before_ready_resumes_the_predecessor() {
        for session_id in [None, Some(uuid::Uuid::from_u128(10))] {
            let (_dir, _kernel, mut successor, transfer, handoff, background) =
                begin_handoff(session_id);
    ...
    fn undeliverable_abort_to_a_live_successor_retires_the_predecessor() {
        for session_id in [None, Some(uuid::Uuid::from_u128(11))] {
            let (_dir, _kernel, successor, transfer, handoff, background) =
                begin_handoff(session_id);
            drop(successor);
            assert!(handoff.join().unwrap().unwrap());
            drop(transfer);
