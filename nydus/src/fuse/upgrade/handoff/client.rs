...
use super::super::lifecycle::handoff_deadline;
...
pub(in crate::fuse::upgrade) fn probe_existing_instance(
    ...
    let (info, _session_id) = client
        .info_until(ControlClient::info_deadline())
        .context("control INFO failed; instance ownership remains unknown")?;
    Ok(Some(info))
}
...
impl Handoff {
    ...
    fn commit_with_timeout(&mut self, timeout: std::time::Duration) -> Result<()> {
        ...
        // READY ends the bounded preparation phase. An ambiguous verdict must
        // keep workers parked until predecessor exit is confirmed, even if
        // that takes longer than HANDOFF_TIMEOUT. Only explicit ABORT rolls back.
        loop {
            ...
            match read_frame_until::<Response>(&mut self.client.stream, cutover) {
                ...
                outcome => {
                    warn!(
                        predecessor_pid = self.client.peer_pid,
                        "ambiguous fuse handoff verdict; successor remains parked \
                         until predecessor exit is confirmed: {outcome:?}"
                    );
                    match self
                        .client
                        .peer_process
                        .terminate_and_confirm(CONTROL_RESPONSE_TIMEOUT)
                    {
                        Ok(true) => {
                            info!(
                                predecessor_pid = self.client.peer_pid,
                                "fuse predecessor exit confirmed; successor may resume"
                            );
                            return Ok(());
                        }
                        Ok(false) => {}
                        Err(err) => warn!(
                            "failed to retire fuse predecessor; successor remains parked: {err}"
                        ),
                    }
                }
            }
...
mod tests {
        ...
        let (_dir, path, me, server) =
            predecessor_endpoint(Some(Uuid::from_u128(11)), |stream, info| {
                ...
                let metadata = SessionTransferMetadata {
                    session_id: Some(Uuid::from_u128(12)),
    ...
    #[test]
    fn standalone_handoff_receives_one_fd_and_validates_the_connection() {
        let (_dir, path, me, server) = predecessor_endpoint(None, |stream, info| {
            write_frame_until(stream, test_deadline(), &Response::HandoffTransfer {}).unwrap();
            let metadata = SessionTransferMetadata {
                session_id: None,
                mountpoint: info.mountpoint.clone(),
        ...
        let error = begin_handoff(&path, &me)
            .err()
            .expect("the standalone transfer must reach FUSE descriptor validation");
        assert!(
            error.report().to_string().contains("/dev/fuse"),
            "unexpected error: {error:?}"
        );
        server.join().unwrap();
    }
