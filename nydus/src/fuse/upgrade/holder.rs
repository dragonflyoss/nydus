...
use super::identity::{authenticate_peer, InstanceInfo};
use super::lifecycle::SessionRuntimeHandle;
...
pub(super) fn retain_session_transfer(
    ...
    let session_id = transfer.session_id().ok_or_else(|| {
        Error::InvalidParameter("cannot retain a Standalone Session for crash recovery".to_string())
    })?;
...
mod tests {
    ...
    fn fresh_session_sends_opaque_transfer_before_holder_acknowledgement() {
        ...
        let holder = std::thread::spawn(move || {
            let (mut stream, _) = listener.accept().unwrap();
            let opaque = receive_opaque_transfer(&mut stream, 2, holder_deadline()).unwrap();
    ...
    fn fresh_session_surfaces_holder_rejection() {
        ...
        let holder = std::thread::spawn(move || {
            let (mut stream, _) = listener.accept().unwrap();
            let _ = receive_opaque_transfer(&mut stream, 2, holder_deadline()).unwrap();
            write_frame_until(
                &mut stream,
                holder_deadline(),
    ...
    #[test]
    fn standalone_retention_is_rejected_before_connecting_to_a_holder() {
        let dir = tempfile::tempdir().unwrap();
        let (handle, background, _kernel) = parked_runtime_handle(None);
        let error =
            retain_session_transfer(&dir.path().join("missing.sock"), &recovery_info(), &handle)
                .expect_err("standalone sessions cannot be retained for failover");
    ...
    #[test]
    fn recovery_rejects_standalone_metadata_even_with_two_descriptors() {
        let mut metadata: serde_json::Value =
            serde_json::from_slice(&valid_metadata(Uuid::from_u128(7))).unwrap();
        metadata["session_id"] = serde_json::Value::Null;
        ...
        assert!(error.contains("protection"), "{error}");
