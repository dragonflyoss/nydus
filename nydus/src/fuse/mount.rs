...
use super::service::SessionOrigin;
...
///
/// * `relinquished` — this process gave up read and unmount ownership, either
///   after READY or because ABORT could not be delivered safely. This does
///   not prove that a successor is serving. The mount device never changed,
///   so only the lifecycle can establish that this process must not unmount.
///   Join the exited workers and leave.
...
/// device is stable across an fd handoff (which `relinquished` covers) and
...
pub(super) fn finish_session(
    ...
    if !relinquished && owned_mount_present {
    ...
    // Ownership relinquished, or the mount at the path is no longer the one
    // recorded (detached, replaced, or unidentifiable): never unmount by
    // path. Disarm the mount teardown (a no-op if already disarmed) so the
    // drop inside join never unmounts, release
