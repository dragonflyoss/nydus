...
use super::upgrade::{
    validate_fuse_connection, FuseInitState, SessionProtection, SessionRuntimeHandle,
    SessionTransfer, HANDOFF_TIMEOUT,
};
...
impl FuseService {
    ...
    pub(super) fn recover(
        ...
        let session_id = transfer.session_id().ok_or_else(|| {
            Error::InvalidParameter("cannot recover a Standalone Session".to_string())
        })?;
        let session = Self::adopt_transfer(fs, mountpoint, config, transfer, HANDOFF_TIMEOUT)
    ...
    /// Imports a [`SessionTransfer`] into a paused session, preserving its
    /// protection. Adoption of an already-initialized FUSE connection belongs to
    ...
    fn adopt_transfer(
        ...
        validate_fuse_connection(fuse_fd.as_fd()).context("invalid handed-over FUSE connection")?;
        let state = metadata.fuse_session_state;
    ...
    pub(super) fn runtime_handle(&self) -> SessionRuntimeHandle {
        self.handle.clone()
    }
    ...
    pub(super) fn abort_adoption(self) -> Result<()> {
    ...
    pub(super) fn shutdown_unpublished(self) -> Result<()> {
        ...
        let relinquished = handle.lifecycle().is_relinquished();
        finish_session(
            session,
            &mountpoint,
            origin,
    ...
    pub fn serve(self) -> Result<std::io::Result<()>> {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                // corpse. `claim_for_teardown` returns immediately once this
                // process has relinquished ownership.
                let teardown = |session: fuser::BackgroundSession| {
                    ...
                    finish_session(
                        ...
                        &mountpoint,
                        origin,
                        lifecycle.is_relinquished(),
                        mount_dev.as_deref(),
                    )
                };
                // Relinquishment ends this process's ownership even if the
                // connection remains live. POLLERR detects kernel unmount or
                // abort before workers finish; is_finished also catches worker
                // exit while a retained connection is still live.
...
mod tests {
    ...
    fn teardown_waits_for_the_handoff_coordinator_to_resolve() {
        for relinquished in [true, false] {
            ...
            lifecycle.resolve_for_test(relinquished);
            done_rx.recv_timeout(Duration::from_secs(1)).unwrap();
            thread.join().unwrap();
            assert_eq!(lifecycle.is_relinquished(), relinquished);
            assert_eq!(lifecycle.is_shutting_down(), !relinquished);
        }
    }
