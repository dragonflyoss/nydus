...
//! text exposition produced by [`nydus_core::metrics`]) and returns `404` for
...
async fn handle_request(
    ...
    let response = if req.method() == Method::GET && req.uri().path() == "/metrics" {
        let body = nydus_core::metrics::encode_text();
        Response::builder()
        ...
        let body = nydus_core::metrics::trace::encode_json();
        Response::builder()
            .status(StatusCode::OK)
