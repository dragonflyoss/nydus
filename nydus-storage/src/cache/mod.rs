...
pub const REDIRECT_PREFETCH_RAMP_GROUPS: usize = 16;
...
pub fn decode_group_from_window(
    ...
    decoded.clear();
    if is_stored_plain_group(blob_metadata, group) {
...
pub fn fetch_decode_validate_group_into<'a>(
...
pub fn validate_group_with_metrics(
    backend: &Arc<dyn BlobBackend>,
    group: &BlobMetadataGroup,
    decoded: &[u8],
) -> io::Result<()> {
...
fn is_stored_plain_group(blob_metadata: &BlobMetadata, group: &BlobMetadataGroup) -> bool {
    blob_metadata.compressor() == BlobMetadataCompressor::None
        || u64::from(group.compressed_size()) == group.uncompressed_byte_size()
}

pub fn validate_decoded_group(group: &BlobMetadataGroup, decoded: &[u8]) -> io::Result<()> {
...
pub fn is_group_crc_mismatch(err: &io::Error) -> bool {
    err.get_ref()
...
mod tests {
    use super::*;
    use nydus_format::blob::BLOB_METADATA_DEFAULT_CHUNK_SIZE;
    use nydus_format::erofs::EROFS_BLOCK_SIZE;

    fn group(uncompressed_block_offset: u64, uncompressed_block_count: u32) -> BlobMetadataGroup {
    ...
    fn plan_prefetch_batches_keeps_one_group_per_window_at_default_target() {
        let blocks = BLOB_METADATA_DEFAULT_CHUNK_SIZE / EROFS_BLOCK_SIZE;
        let groups = vec![
            group(0, blocks),
            group(blocks as u64, blocks),
            group(2 * blocks as u64, blocks),
        ];
        let batches = plan_prefetch_batches(&groups, BLOB_METADATA_DEFAULT_CHUNK_SIZE as u64);
