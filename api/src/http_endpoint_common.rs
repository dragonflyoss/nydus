...
mod tests {
    ...
    fn test_metrics_backend_handler_get() {
        ...
        let result = handler.handle_request(
            &req,
            &|_| Ok(ApiResponsePayload::BackendMetrics("{}".to_string())),
        );
        assert!(result.is_ok());
    }
    ...
    fn test_metrics_blobcache_handler_get() {
        ...
        let result = handler.handle_request(
            &req,
            &|_| Ok(ApiResponsePayload::BlobcacheMetrics("{}".to_string())),
        );
        assert!(result.is_ok());
    }
    ...
    fn test_mount_handler_put_with_body() {
        ...
        let body = r#"{"source":"/path","config":"{}"}"#;
        let req =
            req_with_body("PUT", "http://localhost/api/v1/mount?mountpoint=/test", body);
        let result = handler.handle_request(&req, &|_| ok_empty());
        assert!(result.is_ok());
    }
