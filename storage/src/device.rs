...
mod tests {
    ...
    fn test_blob_info_setter_getter_pairs() {
        let mut blob_info = BlobInfo::new(
            0, "test-blob".to_owned(), 1024, 512, 1 << 20, 1, BlobFeatures::empty(),
        );
    ...
    fn test_blob_info_compressor_and_digester() {
        let mut blob_info = BlobInfo::new(
            0, "c-d-blob".to_owned(), 0, 0, 1 << 20, 0, BlobFeatures::empty(),
        );
    ...
    fn test_blob_info_toc_and_meta_digest() {
        let mut blob_info = BlobInfo::new(
            0, "digest-blob".to_owned(), 0, 0, 1 << 20, 0, BlobFeatures::empty(),
        );
