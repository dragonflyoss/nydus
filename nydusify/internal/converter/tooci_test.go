...
// TestRewriteOCIConfigResetsDiffIDsAndDropsNydusHistory verifies that the
// reverse conversion restores an ordinary OCI config: the rebuilt layers get
// fresh diff ids, the history entries the nydus conversion appended are gone,
// and every other field survives untouched.
func TestRewriteOCIConfigResetsDiffIDsAndDropsNydusHistory(t *testing.T) {
	source := []byte(`{
		"architecture": "amd64",
		"os": "linux",
		"config": {
	...
	out, err := rewriteOCIConfig(source, diffIDs)
	if err != nil {
...
func TestNydusDataLayersRejectsNonNydusManifests(t *testing.T) {
	...
	if _, err := nydusDataLayers([]ocispec.Descriptor{blob}); err == nil {
		t.Error("expected an error when the bootstrap layer is missing")
	}
	blobs, err := nydusDataLayers([]ocispec.Descriptor{blob, blob, bootstrap})
	if err != nil {
		t.Fatalf("nydusDataLayers: %v", err)
	}
	if len(blobs) != 2 {
		t.Errorf("got %d data layers, want 2", len(blobs))
	}
}
