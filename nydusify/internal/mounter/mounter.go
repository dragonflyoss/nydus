...
// Package mounter materializes a single image (OCI or nydus) at a local
// mountpoint for the mount command.
package mounter
...
// Mounter materializes a single image at a local mountpoint. Nydus images are
...
// nydus image it runs `nydus fuse` in the foreground and blocks until the
...
func (m *Mounter) Mount(ctx context.Context) error {
	...
	img, err := nydusfs.LoadImage(ctx, provider, m.opt.Target, m.opt.PlatformMC, remote.PullAll, remote.Target)
	if err != nil {
...
func (m *Mounter) extractOCI(ctx context.Context, cs content.Store, img *nydusfs.Image) error {
	log.G(ctx).Infof("extracting OCI image %s to %s", img.Ref, m.opt.Mountpoint)
	if err := nydusfs.ApplyOCIImage(ctx, cs, img, m.opt.Mountpoint); err != nil {
...
// mountNydus materializes the nydus bootstrap, generates a registry-backed
// storage config, and runs `nydus fuse` in the foreground mounting the image at
...
// unmounted cleanly.
func (m *Mounter) mountNydus(ctx context.Context, provider *remote.Provider, cs content.Store, img *nydusfs.Image, scratchDir string) error {
	...
	configYAML, err := nydusfs.WriteRegistryConfig(provider, remote.Target, img, cacheDir, configPath, m.opt.Prefetch)
	if err != nil {
		return errors.Wrap(err, "generate storage config")
	}
	log.G(ctx).Debugf("generated nydus storage config %s:\n%s", configPath, configYAML)
	...
	cmd := exec.Command(cmp.Or(m.opt.BuilderPath, nydus.DefaultBuilder), args...)
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	if err := cmd.Start(); err != nil {
		return errors.Wrap(err, "start nydus fuse")
	}
	log.G(ctx).Infof("mounting nydus image %s at %s (metrics at unix://%s, press Ctrl+C to unmount)", img.Ref, m.opt.Mountpoint, apiSocket)

	// `nydus fuse` runs in the foreground and performs its own unmount on
	...
	case err := <-waitErr:
		if err != nil {
			return errors.Wrap(err, "nydus fuse exited")
		}
	}
	log.G(ctx).Infof("unmounted nydus image at %s", m.opt.Mountpoint)
	return nil
}
