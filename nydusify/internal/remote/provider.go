...
// PushBlobFile uploads one blob directly from path without first copying it
// into the provider's local content store.
func (p *Provider) PushBlobFile(ctx context.Context, path string, desc ocispec.Descriptor, ref string) error {
	normalized, err := normalizeRef(ref)
	if err != nil {
		return err
	}
	return pushBlobFile(ctx, p.resolver(Target), desc, normalized, path)
}
