...
// LocalBlob is a writer-produced blob that is referenced by the artifact
// manifest but remains at its original path instead of being copied into the
// nydusify content store.
type LocalBlob struct {
	Path       string
	Descriptor ocispec.Descriptor
}

// ArtifactResult contains the manifest assembled in the content store and the
// data blobs that must be available in the target repository before it is
// pushed. External blobs already belong to the parent repository; local blobs
// must be uploaded directly from Path.
...
// files into an OCI nydus image rooted at a single manifest descriptor. It
// leaves local data blobs at their source paths so callers can upload them
// directly instead of copying them through the content store.
func ConvertNydusArtifacts(ctx context.Context, cs content.Store, opt ArtifactOption) (*ArtifactResult, error) {
	if opt.BootstrapPath == "" {
		return nil, errors.New("--bootstrap is required when packaging nydus artifacts")
	}
	if opt.Platform.OS == "" {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "read bootstrap %q", opt.BootstrapPath)
	}
	...
	if len(blobPaths) == 0 && len(opt.ParentImages) == 0 {
		return nil, errors.New("at least one --blob/--blob-dir or --parent-image is required")
	}
	if len(opt.ParentImages) > 1 {
		return nil, errors.New("only one --parent-image is supported")
	}
	referenced, err := inspectBootstrapBlobDigests(ctx, opt.BuilderPath, opt.BootstrapPath)
	if err != nil {
		return nil, err
	}
	blobPaths = filterArtifactBlobPaths(blobPaths, referenced)
	...
	localBlobs := make([]LocalBlob, 0, len(blobPaths))
	...
	for _, path := range blobPaths {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "inspect blob %q", path)
		}
		blobDescs = append(blobDescs, desc)
		blobMetas = append(blobMetas, meta)
		localBlobs = append(localBlobs, LocalBlob{Path: path, Descriptor: desc})
	}
	blobDescs, blobMetas, externalBlobs, localBlobs, err = filterArtifactBlobsByReferences(referenced, blobDescs, blobMetas, externalBlobs, localBlobs)
	if err != nil {
		return nil, err
	}
	...
	manifestDesc, err := writeNydusManifest(ctx, cs, blobDescs, *bootstrapDesc, baseConfig, opt.Platform, "Nydus Artifact")
	if err != nil {
		return nil, err
	}
...
func readBootstrapBlobMetas(ctx context.Context, cs content.Store, desc ocispec.Descriptor) ([]nydus.BlobMetaFile, error) {
	...
	err := oci.WalkBootstrapLayer(ctx, cs, desc, func(hdr *tar.Header, tr *tar.Reader) error {
		...
		if err != nil {
			return errors.Wrapf(err, "read parent blob meta %s", hdr.Name)
		}
...
func filterArtifactBlobsByReferences(
	referenced map[digest.Digest]struct{},
	blobDescs []ocispec.Descriptor,
	blobMetas []nydus.BlobMetaFile,
	externalBlobs []ExternalBlob,
	localBlobs []LocalBlob,
) ([]ocispec.Descriptor, []nydus.BlobMetaFile, []ExternalBlob, []LocalBlob, error) {
	if len(referenced) == 0 {
		return blobDescs, blobMetas, externalBlobs, localBlobs, nil
	}
	...
	for dgst := range referenced {
		if _, ok := provided[dgst]; !ok {
			return nil, nil, nil, nil, errors.Errorf("bootstrap references missing nydus blob %s; pass it with --blob/--blob-dir or include it through --parent-image", dgst)
		}
	}
	...
	filteredLocal := localBlobs[:0]
	for _, blob := range localBlobs {
		if _, ok := referenced[blob.Descriptor.Digest]; ok {
			filteredLocal = append(filteredLocal, blob)
		}
	}
	return filteredDescs, filteredMetas, filteredExternal, filteredLocal, nil
}
...
func filterArtifactBlobPaths(paths []string, referenced map[digest.Digest]struct{}) []string {
	if len(referenced) == 0 {
		return paths
	}
	filtered := paths[:0]
	for _, path := range paths {
		dgst, ok := artifactBlobDigestFromPath(path)
		if !ok {
			// Preserve compatibility with explicitly supplied, non-digest-named
			// blobs. describeArtifactBlob hashes them before the final filter.
			filtered = append(filtered, path)
			continue
		}
		if _, ok := referenced[dgst]; ok {
			filtered = append(filtered, path)
		}
	}
	return filtered
...
func describeArtifactBlob(path string) (ocispec.Descriptor, nydus.BlobMetaFile, error) {
	f, err := os.Open(path)
	if err != nil {
		return ocispec.Descriptor{}, nydus.BlobMetaFile{}, errors.Wrap(err, "open blob")
	}
	defer func() { _ = f.Close() }()
	info, err := f.Stat()
	if err != nil {
		return ocispec.Descriptor{}, nydus.BlobMetaFile{}, errors.Wrap(err, "stat blob")
	}
	...
	if !ok {
		...
		if _, err := io.Copy(digester.Hash(), f); err != nil {
			return ocispec.Descriptor{}, nydus.BlobMetaFile{}, errors.Wrap(err, "hash non-digest-named blob")
		}
		dgst = digester.Digest()
	}

	meta, err := os.ReadFile(path + ".blob.meta")
	if errors.Is(err, os.ErrNotExist) {
		meta, err = nydus.ExtractBlobMeta(f, info.Size())
	}
	if err != nil {
		return ocispec.Descriptor{}, nydus.BlobMetaFile{}, errors.Wrap(err, "read blob metadata")
	}
...
func artifactBlobDigestFromPath(path string) (digest.Digest, bool) {
	name := filepath.Base(path)
	if !isSha256Hex(name) {
		return "", false
	}
	dgst, err := digest.Parse("sha256:" + name)
	return dgst, err == nil
}
