...
/// A blob-relative range to resolve into [`FdRange`]s.
#[derive(Clone, Copy)]
...
pub(crate) fn checked_range_end(offset: u64, len: u64) -> Result<Option<u64>> {
    if len == 0 {
        return Ok(None);
    }
...
pub(crate) fn push_blob_fd_ranges(
    reader: &ErofsReader,
    zero_fd: RawFd,
...
/// Append `range`, merging it into the previous range when both are backed by
/// the same fd and contiguous (adjacent zero-fd ranges also merge).
pub(crate) fn push_fd_range(ranges: &mut Vec<FdRange>, range: FdRange, zero_fd: RawFd) {
    if range.len == 0 {
        return;
    }
