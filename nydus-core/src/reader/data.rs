...
impl ErofsReader {
    ...
    pub fn read_z_inode_tail<'a>(
        ...
        let inode_offset = self.z_inode_offset(nid)?;
        let header_size = inode.header_size() + inode.xattr_size();
        let tail_offset = align_up_usize(header_size, 8)
            .and_then(|size| inode_offset.checked_add(size))
            .ok_or_else(|| {
                io::Error::new(io::ErrorKind::InvalidData, "z_erofs tail offset overflow")
            })?;
        let head = self
            .mmap_slice(tail_offset, Z_EROFS_MAP_HEADER_SIZE)
        ...
        let len = usize::try_from(inode.size().div_ceil(EROFS_BLOCK_SIZE as u64))
            .ok()
            .and_then(|count| count.checked_mul(Z_EROFS_LCLUSTER_INDEX_SIZE))
            .and_then(|len| len.checked_add(Z_EROFS_MAP_HEADER_SIZE + 8))
            .ok_or_else(|| {
                io::Error::new(io::ErrorKind::InvalidData, "z_erofs index length overflow")
            })?;
    ...
    fn z_inode_offset(&self, nid: u64) -> io::Result<usize> {
        nid.checked_mul(EROFS_SLOTSIZE as u64)
            .and_then(|offset| {
                offset
                    .checked_add(self.superblock().meta_blkaddr() as u64 * EROFS_BLOCK_SIZE as u64)
            })
            .and_then(|offset| usize::try_from(offset).ok())
            .ok_or_else(|| {
                io::Error::new(io::ErrorKind::InvalidData, "z_erofs inode offset overflow")
            })
    }

    fn z_packed_inode(&self) -> io::Result<(u64, ErofsInode<'_>)> {
    ...
    fn write_z_data_to(
        ...
        let end = offset
            ...
            .ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "z_erofs read exceeds inode size",
                )
            })?;
        if size == 0 {
            return Ok(0);
        ...
        if head & Z_EROFS_FRAGMENT_INODE_FLAG != 0 {
            if depth > 0 || self.superblock().packed_nid() == Some(nid) {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "packed inode is itself a fragment",
                ));
            }
            let (packed_nid, packed) = self.z_packed_inode()?;
            let frag_off = head & !Z_EROFS_FRAGMENT_INODE_FLAG;
            frag_off
                .checked_add(inode.size())
                .filter(|&end| end <= packed.size())
                .ok_or_else(|| {
                    io::Error::new(
                        io::ErrorKind::InvalidData,
                        "fragment exceeds packed inode size",
                    )
                })?;
        ...
        let tail_fragment = if h_advise & Z_EROFS_ADVISE_FRAGMENT_PCLUSTER != 0 {
            if depth > 0 || self.superblock().packed_nid() == Some(nid) {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    "packed inode has a tail fragment",
                ));
            }
            ...
            let frag_off = (u64::from(index_at(head).1) << 32) | u64::from(lo);
            let (_, packed) = self.z_packed_inode()?;
            frag_off
                .checked_add(file_size - head as u64 * block)
                .filter(|&end| end <= packed.size())
                .ok_or_else(|| {
                    io::Error::new(
                        io::ErrorKind::InvalidData,
                        "tail fragment exceeds packed inode size",
                    )
                })?;
            Some((head, frag_off))
        } else {
        ...
        Z_BUFS.with(|cell| -> io::Result<()> {
            ...
            while i < nlclusters {
                ...
                if logical_start + logical_len as u64 <= offset {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "z_erofs pcluster does not cover read offset",
                    ));
                }
                ...
                if phys_len == 0 {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "z_erofs pcluster has no physical blocks",
                    ));
                }
                if kind == Z_EROFS_LCLUSTER_TYPE_PLAIN && logical_len > phys_len {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "z_erofs plain extent exceeds its physical length",
                    ));
                }
                ...
                let device = &blob_layout[usize::from(blob_index) - 1];
                if blob_off + phys_len as u64 > device.blocks * block {
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        "z_erofs pcluster exceeds device size",
                    ));
                }
                let key = PclusterKey {
                    blob_index,
