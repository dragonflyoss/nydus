...
//!
//! Access patterns come from the apiserver `/trace` endpoint of a running
//! `nydus fuse` mount ([`load_patterns_from_apiserver`]) or from a saved JSON
//! trace document ([`load_patterns_from_file`]); [`build_ondemand_blob`] then
//! pulls the accessed groups through the regular blob cache and assembles the
//! ondemand artifact and bootstrap in memory.
...
use crate::telemetry::trace::{TraceDocument, TraceEntry, TRACE_DOCUMENT_VERSION};
...
/// Parse a `unix:///path/to.sock` apiserver address into its socket path.
/// This is the address syntax shared by the apiserver itself and clients like
/// [`load_patterns_from_apiserver`].
