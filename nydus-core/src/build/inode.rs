...
pub fn choose_epoch(inodes: &[InodeInfo]) -> u64 {
...
///
/// Implemented by the host-filesystem walker below and by in-memory trees in
/// higher-level callers, so building a directory and merging layers share a
/// single flattening pass and cannot drift apart. `C` is the traversal
