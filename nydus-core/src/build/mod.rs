...
use crate::metadata::*;
use crate::utils::{align_up, sha256_bytes, write_zero_padding};
...
pub struct DirImageOptions<'a> {
    ...
    pub compressor: BlobMetaCompressor,
...
impl DirImageOptions<'_> {
    ...
    pub fn validate(&self) -> Result<()> {
        // Validate EROFS file chunk size. BlobMeta groups are formed
...
pub fn build_dir_image(options: &DirImageOptions<'_>, blob_out: File) -> Result<DirImage> {
    options.validate()?;
    let chunkbits = options.chunk_size.trailing_zeros();
