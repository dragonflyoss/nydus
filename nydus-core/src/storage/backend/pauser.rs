//! Per-backend pauser used to temporarily halt a backend's network reads.
//!
...
//! plane (or rate-limit handler) can call [`Pauser::pause_for`] to stall that
//! backend's traffic for a while, e.g. to back off after a burst of `429`
//! responses. Each backend owns its pauser, so several backends in one process
//! throttle independently. No HTTP/management trigger is wired up yet — only
//! the API is provided.
