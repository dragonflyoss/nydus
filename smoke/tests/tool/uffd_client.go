...
type UffdClient struct {
	conn       *net.UnixConn
	uffdFd     int
	memSlice   []byte // mmap'd region backed by this slice
	memPtr     uintptr
	memSize    uint64
	deviceSize uint64
	blockSize  uint32
...
func uffdWake(fd int, addr uintptr, len uint64) error {
	...
	range_.Start = uint64(addr)
	range_.Len = len

	_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), uffdioWake, uintptr(unsafe.Pointer(&range_)))
	if errno != 0 {
...
func (c *UffdClient) Handshake(policy int, enablePrefault bool) error {
	...
	c.memPtr = addr
	c.memSize = mmapSize
	// Keep a slice reference so Munmap works via syscall.Munmap.
	c.memSlice = unsafe.Slice((*byte)(unsafe.Pointer(addr)), mmapSize)
...
func (c *UffdClient) zerocopyWorker() {
	...
	for {
		...
		for i, range_ := range resp.Ranges {
			fd := fds[i]
			targetAddr := c.region.BaseHostVirtAddr + (range_.BlockOffset - c.region.Offset)
			...
			uffdWake(c.uffdFd, uintptr(targetAddr), range_.Len)
		}
	}
}
...
func (c *UffdClient) ReadAt(offset, length int64) ([]byte, error) {
	...
	srcSlice := unsafe.Slice((*byte)(unsafe.Pointer(start)), length)
	copy(data, srcSlice)
	return data, nil
}
