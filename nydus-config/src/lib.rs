...
pub struct HttpConfig {
    ...
    /// runtime is shared by every registry backend in the process and is
    /// built on the first read, so only the configuration of the first
    /// backend that reads can take effect; a later, different value is
    /// logged and ignored.
    #[serde(default)]
