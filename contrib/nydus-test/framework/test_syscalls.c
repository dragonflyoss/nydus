...
static char testxattr_no_kv[1024];
static char testxattr_empty_value[1024];
static char testxattr_insufficient_buffer[1024];
...
static int test_xattr()
{
	char list[256];
	int res;
	start_test("xattr");

	res = listxattr(testxattr_no_kv, list, 256);
	if (res < 0)
	{
		ERROR("listxattr failed err: %d", errno);
		return -1;
	}

	res = getxattr(testxattr_empty_value, "trusted.nydus.key", list, 256);
	if (res < 0)
	{
		ERROR("getxattr failed err: %d", errno);
		return -1;
	}
	else if (res > 0)
	{
		ERROR("getxattr value is not empty");
		return -1;
	}

	res = getxattr(testxattr_insufficient_buffer, "trusted.nydus.key", list, 0);
	if (res < 0)
	{
		if (errno != ERANGE)
		{
	...
	res = getxattr(testxattr_insufficient_buffer, "trusted.nydus.key", list, 2);
	if (res < 0)
	{
		if (errno != ERANGE)
		{
	...
	res = getxattr(testxattr_insufficient_buffer, "trusted.nydus.nokey", list, 2);
	if (res < 0)
	{
		if (errno != ENOATTR)
		{
...
int main(int argc, char *argv[])
{
	...
	sprintf(testxattr_no_kv, "%s/xattr_no_kv", realpath);
	sprintf(testxattr_empty_value, "%s/xattr_empty_value", realpath);
	sprintf(testxattr_insufficient_buffer, "%s/xattr_insufficient_buffer", realpath);
