...
func TestNew(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		parserPatches := gomonkey.ApplyFunc(parser.New, func(r *remote.Remote, interestedArch string) (*parser.Parser, error) {
			require.Equal(t, "amd64", interestedArch)
	...
	t.Run("default remote error", func(t *testing.T) {
		patches := gomonkey.ApplyFunc(providerpkg.DefaultRemote, func(string, bool) (*remote.Remote, error) {
			return nil, errors.New("remote error")
		})
		defer patches.Reset()

		_, err := New(Opt{Sources: []string{"repo/a:latest"}, ExpectedArch: "amd64"})
