...
import (
	...
	snapshotterReconverter "github.com/containerd/nydus-snapshotter/pkg/reconverter"
)

// ReverseConvert converts Nydus image to OCI image
func ReverseConvert(ctx context.Context, opt Opt) error {
	start := time.Now()
	logrus.Warn("note: conversion from OCI v1 to nydus is currently experimental")
	...
	dstDesc, err := reconvertWithSnapshotter(ctx, pvd, platformMC, tmpDir, []ocispec.Descriptor{*rootDesc}, opt)
	if err != nil {
...
func reconvertWithSnapshotter(
	...
	logrus.Debugf("Start reconvertWithSnapshotter: src_count=%d workDir=%s", len(srcDescs), workDir)
	cs := pvd.ContentStore()
	...
	indexFn := snapshotterReconverter.DefaultIndexConvertFunc(layerFn, false, platformMC)

	// The source may include multiple descriptors (e.g., manifests). Convert each and pick the first valid.
	for _, src := range srcDescs {
		logrus.Debugf("Converting src descriptor: mediaType=%s digest=%s", src.MediaType, src.Digest.String())
		dst, err := indexFn(ctx, cs, src)
		if err != nil {
			return nil, err
		}
		if dst != nil {
	...
	return nil, errors.New("no convertible descriptor found")
}
