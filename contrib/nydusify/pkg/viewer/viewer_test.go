...
func TestHandleExternalBackendConfig(t *testing.T) {
	...
	t.Run("Run read file failed", func(t *testing.T) {
		osStatPatches := gomonkey.ApplyFunc(os.Stat, func(string) (os.FileInfo, error) {
			return nil, nil
		})
		defer osStatPatches.Reset()
		osReadFilePatches := gomonkey.ApplyFunc(os.ReadFile, func(string) ([]byte, error) {
			return nil, errors.New("read file failed mock error")
		})
		defer osReadFilePatches.Reset()
		err := fsViwer.handleExternalBackendConfig()
		assert.Error(t, err)
	})
	...
	t.Run("Run normal", func(t *testing.T) {
		...
		defer osStatPatches.Reset()
		osReadFilePatches := gomonkey.ApplyFunc(os.ReadFile, func(string) ([]byte, error) {
			return bkdConfig, nil
		})
		defer osReadFilePatches.Reset()

		osWriteFilePatches := gomonkey.ApplyFunc(os.WriteFile, func(string, []byte, os.FileMode) error {
			return nil
		})
		defer osWriteFilePatches.Reset()
		err := fsViwer.handleExternalBackendConfig()
		assert.NoError(t, err)
	})
}
