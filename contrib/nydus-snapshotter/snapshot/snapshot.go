...
func NewSnapshotter(ctx context.Context, cfg *config.Config) (snapshots.Snapshotter, error) {
	...
	return &snapshotter{
		...
		manager:               pm,
		hasDaemon:             hasDaemon,
		disableNydusOverlayFS: cfg.DisableNydusOverlayFS,
	}, nil
}
...
func (o *snapshotter) remoteMounts(ctx context.Context, s storage.Snapshot, id string, labels map[string]string) ([]mount.Mount, error) {
	...
	// when hasDaemon and disableNydusOverlayFS, return overlayfs mount slice
	if o.disableNydusOverlayFS && o.hasDaemon {
	...
	nydusOpt := &NydusOption{
		Source:      source,
		Config:      configContent,
		Snapshotdir: o.snapshotDir(s.ID),
	}
	opt, err := json.Marshal(nydusOpt)
	if err != nil {
		return nil, errors.Wrapf(err, "remoteMounts: failed to marshal NydusOption")
	}
	options := append(overlayOptions, fmt.Sprintf("nydusoption=%s", string(opt)))
