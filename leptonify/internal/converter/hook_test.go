...
// identical to the source. This guards the `leptonify check` config-consistency
...
func TestRewriteBootstrapConfigPreservesRuntimeConfig(t *testing.T) {
	...
	if last.CreatedBy != "Lepton Converter" || last.Comment != "Lepton Bootstrap Layer" {
		t.Errorf("unexpected bootstrap history entry: %#v", last)
	}
}

// TestRewriteBootstrapConfigKeepsUnknownFields verifies that fields the lepton
