...
// Index/manifest/config descriptors and (for lepton images) the bootstrap
...
// the lepton bootstrap layer.
//
// Filtering rules per layer:
//   - lepton bootstrap layer: always kept (needed for the static check and the
//     metadata export);
//   - lepton data blob layer: kept only when opts.PullLeptonBlobs is true;
...
func selectLayersHandler(h images.HandlerFunc, opts PullOptions) images.HandlerFunc {
	return func(ctx context.Context, desc ocispec.Descriptor) ([]ocispec.Descriptor, error) {
		...
		for _, child := range children {
			switch {
			case converter.IsLeptonBootstrap(child):
				filtered = append(filtered, child)
			case converter.IsLeptonBlob(child):
				if opts.PullLeptonBlobs {
					filtered = append(filtered, child)
				}
