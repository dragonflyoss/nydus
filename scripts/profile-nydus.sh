#!/bin/bash
# profile-nydus.sh — Automated flamegraph profiling for `nydus mount`
#
# Usage:   sudo ./scripts/profile-nydus.sh
# Output:  /tmp/profile-nydus/flame.svg
#
# Prerequisites:
...
cleanup() {
    ...
    # Kill nydus mount if still running
...
[[ -x "${NYDUS}" ]]    || die "${NYDUS} not found — run: cargo build --release"
...
# ── build nydus image ───────────────────────────────────────────────
info "Building nydus image (chunksize=1M)..."
"${NYDUS}" build "${IMG}" --blobdev "${BLOB}" --chunksize 1048576 "${CORPUS_DIR}"

# ── mount nydus ─────────────────────────────────────────────────────
info "Mounting nydus (threads=${FUSE_THREADS})..."
"${NYDUS}" mount "${IMG}" "${MNT}" --blobdev "${BLOB}" --threads "${FUSE_THREADS}" &
FUSE_PID=$!
...
mountpoint -q "${MNT}" || die "nydus mount failed to mount within 10s"
...
perf script -i "${PERF_DATA}" | \
    ...
    inferno-flamegraph --title "nydus mount sequential read 128K (threads=${FUSE_THREADS})" \
    > "${FLAME_SVG}"
...
info "  PERF_DURATION=30 FUSE_THREADS=4 sudo ./scripts/profile-nydus.sh"
