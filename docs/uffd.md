# Lepton UFFD Service and Wire Protocol
...
a Lepton image to a guest through virtio-pmem.
...
userfaultfd, and sends the userfaultfd to Lepton. Lepton resolves the flattened
image address space on demand.

```text
microVM process -- SCM_RIGHTS(userfaultfd) --> lepton uffd
...
Zerocopy: lepton uffd -- FD ranges --> microVM process
                                         |
...
Copy:     lepton uffd -- UFFDIO_COPY / UFFDIO_ZEROPAGE --> guest thread resumes
```

Two fault policies are supported:

- **Zerocopy**: Lepton returns file descriptors and byte ranges. The microVM
  maps those ranges over the anonymous virtio-pmem VMA and wakes the faulting
  thread.
- **Copy**: Lepton reads the resolved bytes and completes the fault itself with
  `UFFDIO_COPY` or `UFFDIO_ZEROPAGE`. No page response is sent.
...
Lepton presents one byte-addressed device assembled from the bootstrap, blob
...
`PAGE_RESPONSE` defines a `NEXT` bit in `flags`; other messages currently set
...
| `0x81` | `PAGE_RESPONSE` | server to client | Range response |
...
| Offset | Size | Type | Field | Meaning |
|---:|---:|---|---|---|
| 0 | 8 | `u64` | `base_host_virt_addr` | Region base in the microVM process |
| 8 | 8 | `u64` | `size` | Region length in bytes |
| 16 | 8 | `u64` | `offset` | Corresponding flattened-device offset |
| 24 | 8 | `u64` | `page_size` | Fault resolution window |
...
When Zerocopy prefault is enabled, Lepton probes the registered regions and
sends their currently ready ranges as `PAGE_RESPONSE` frames. Prefault does
not download missing blob data. It is processed synchronously so page-response
frames are not written concurrently on the stream. Copy policy ignores the
prefault flag.

## PAGE_RESPONSE

`PAGE_RESPONSE` returns backing file ranges for Zerocopy faults, prefault,
`FETCH`, and `PROBE`.
...
results are split across multiple `PAGE_RESPONSE` frames, so clients must not
assume a fixed batch size.

Bit zero of the PAGE_RESPONSE header `flags` field is `NEXT`:

- `NEXT=1` means another PAGE_RESPONSE for the same logical result follows.
- `NEXT=0` means the current frame is the final batch.
- A single-batch result has `NEXT=0`.
- An empty result is one PAGE_RESPONSE with `range_count=0`, no FDs, and
  `NEXT=0`.
...
```text
host_address = region.base_host_virt_addr
...
`FETCH_REQUEST` asks Lepton to make one device range locally available and
...
Lepton downloads and decodes missing blob groups, then returns one or more
`PAGE_RESPONSE` frames. The returned ranges cover the complete requested
interval without gaps; holes are represented by `/dev/zero` ranges. The client
receives through the PAGE_RESPONSE with `NEXT=0`, then verifies that the
...
The server emits one or more `PAGE_RESPONSE` frames containing:

- the bootstrap range;
- hole ranges backed by `/dev/zero`;
- blob subranges already present in the local cache.

Missing blob ranges are omitted. The PAGE_RESPONSE with `NEXT=0` completes the
probe. If no ranges are ready, that final response has zero ranges and no FDs.

## Fault Handling

For each page-fault event, Lepton finds the registered region containing the
fault address. It aligns the fault offset down to the region's `page_size`
...
3. Send one or more `PAGE_RESPONSE` frames.
...
4. Return to the connection loop without sending `PAGE_RESPONSE`.
...
One `lepton uffd` process accepts multiple client connections. Each connection
...
- `--config` is the regular Lepton storage configuration. It selects the blob
...
- `PAGE_RESPONSE` FD count must equal `range_count`.
...
- PAGE_RESPONSE batches belonging to one logical result are contiguous.
