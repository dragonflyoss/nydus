...
- `--upgrade` directly adopts the session from the daemon already serving the
	mountpoint, without contacting a Holder or changing failover protection.
	Standalone sessions support this handoff. `--supervisor-socket` creates a
	Failover-Protected Session through an external Recovery Holder by retaining the fixed two-fd
...
Options:
	...
	--upgrade
		Hot-upgrade the instance already serving this mountpoint directly, preserving its failover protection. Does not contact a supervisor; requires pidfd process control [env: NYDUS_FUSE_UPGRADE=]
...
- Add `--upgrade` to take over either a matching Standalone or Failover-Protected
  Session directly. The successor inherits the existing protection: standalone
  stays standalone, while protected sessions keep their Session Identity and
  recovery resource. Even if supplied with `--upgrade`, `--supervisor-socket` is
  not contacted and does not enable protection.
- Add `--recover --supervisor-socket <path>` to restore the retained FUSE
  ...
  Session Identity inside the opaque transfer. Standalone sessions cannot
  recover after a crash; enabling failover requires a fresh protected mount.
